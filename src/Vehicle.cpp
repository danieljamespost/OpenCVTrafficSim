...
void Vehicle::drive()
{
    ...
    while (true) {
        ...
        if (timeSinceLastUpdate >= cycleDuration) {
            ...
            // compute current pixel position on street based on driving
            // direction
            ...
            xv = x1 + completion * dx; // new position based on line equation in
                                       // parameter form
            yv = y1 + completion * dy;
            this->setPosition(xv, yv);

            // check wether halting position in front of destination has been
            // reached
            if (completion >= 0.9 && !hasEnteredIntersection) {
                // request entry to the current intersection (using async)
                auto ftrEntryGranted
                    = std::async(&Intersection::addVehicleToQueue,
            ...
            if (completion >= 1.0 && hasEnteredIntersection) {
                ...
                if (streetOptions.size() > 0) {
                    // pick one street at random and query intersection to enter
                    // this street
                ...
                // pick the one intersection at which the vehicle is currently
                // not
                ...
                // send signal to intersection that vehicle has left the
                // intersection
