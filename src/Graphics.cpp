...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects) {
        ...
        if (it->getType() == ObjectType::objectIntersection) {
            ...
            // set color according to traffic light and draw the intersection as
            // a circle
            cv::Scalar trafficLightColor
                = intersection->trafficLightIsGreen() == true
                ? cv::Scalar(0, 255, 0)
                : cv::Scalar(0, 0, 255);
            ...
            cv::circle(
                _images.at(1), cv::Point2d(posx, posy), 50, vehicleColor, -1);
        }
    }

    float opacity = 0.85;
    cv::addWeighted(
        _images.at(1), opacity, _images.at(0), 1.0 - opacity, 0, _images.at(2));
